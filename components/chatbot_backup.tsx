...
export default function Chatbot() {
  ...
  const [desaDataLoaded, setDesaDataLoaded] = useState(false);
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const messagesContainerRef = useRef<HTMLDivElement>(null);
  const desaDataRef = useRef<DesaData[]>([]);

  // Ambil data desa sekali saat komponen pertama kali dimuat
  useEffect(() => {
    const fetchDesaData = async () => {
      try {
        const res = await fetch("http://127.0.0.1:8000/api/tipologi-desa");
        if (res.ok) {
          const data: DesaData[] = await res.json();
          desaDataRef.current = data;
          setDesaDataLoaded(true);
        }
      } catch (err) {
  ...
  // === OPSIONAL: Logika lokal untuk pertanyaan umum ===
  const handleLocalQuery = (query: string): string | null => {
    const data = desaDataRef.current;
    if (!data || data.length === 0) return null;

    const lowerQuery = query.toLowerCase();

    if (lowerQuery.includes("jumlah desa") && lowerQuery.includes("koperasi")) {
      const total = data.filter(d => d.jml_koperasi > 0).length;
      return `Terdapat ${total} desa yang memiliki koperasi.`;
    }

    if (lowerQuery.includes("desa dengan koperasi terbanyak")) {
      const sorted = [...data].sort((a, b) => b.jml_koperasi - a.jml_koperasi);
      const top = sorted[0];
      const namaDesa = top.NAMA_DESA || `${top.NAMA_KEC}, ${top.NAMA_KAB}`;
      return `Desa dengan koperasi terbanyak adalah **${namaDesa}** dengan **${top.jml_koperasi}** koperasi.`;
    }

    if (lowerQuery.includes("total desa")) {
      return `Total desa yang ter **${data.length}**.`;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Coba jawab secara lokal dulu
    const localAnswer = handleLocalQuery(userMessage);
    if (localAnswer) {
      setMessages((prev) => [
        ...prev,
        { role: "user", content: userMessage, timestamp: new Date() },
        { role: "assistant", content: localAnswer, timestamp: new Date() },
      ]);
      return;
    }

    // Jika tidak bisa dijawab lokal, kirim ke AI backend
    setMessages((prev) => [
      ...prev,
      { role: "user", content: userMessage, timestamp: new Date() },
    ]);
    ...
    try {
      const response = await fetch("/api/gemini/chat", {
        ...
        body: JSON.stringify({
          prompt: userMessage,
          desaData: desaDataRef.current, // Kirim data desa ke backend
          ...
          <form onSubmit={handleSubmit} className="p-4 border-t border-gray-700 bg-gray-750">
            <div className="flex gap-2">
              <input
                ...
                placeholder={desaDataLoaded ? "Tanya AI..." : "Memuat data..."}
                ...
                disabled={loading || !desaDataLoaded}
              />
              <button
                type="submit"
                disabled={loading || !desaDataLoaded}
